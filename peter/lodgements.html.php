...
include 'head.php';
?>
<form name="Lodgements" id="Lodgements" method="post"  action="Lodgement.php">             
	<h2>Lodgements <img src="../common/images/help-01.png" alt="Help" id="helpbtn" onmousedown="frameToggleHelp() " > </h2>          
	...
	<fieldset class="halfwidth"> <!--again a boarder-->
		...
		<label for="Home" >Address </label><div  id="Home" name="Home">&nbsp;</br></br></br></br></div>
	</fieldset>
	
	<fieldset class="halfwidth">
		<legend>Lodgements</legend>
		
		<label for="Accountlist">Account type</label>
		<?php include 'lodgement_account_type.php'?>
		
		<label for="AccID">Account ID</label>
		<div  id="AccID" name="AccID">&nbsp;</div>
		<input  type="hidden"  id="AccountID" name="AccountID" placeholder="Enter a current account ID!" pattern="[0-9.]+" hidden> 	
		<!--The above text box is hidden to passs along the account ID when submitting the SQL -->
		
		<label for="Balance">Balance </label>
		<div id="Balance" name="Balance">&nbsp;</div>
		
		<input  type="hidden"  id="AccountType" name="AccountType" placeholder="Enter a current account Type!" pattern="[0-9.]+" hidden> 	
		<!--The above text box is hidden to passs along the account type when submitting the SQL -->
		<br></br></br>
		<label for="Amount" >Lodgement Amount </label>
		<input type="text" id="Amount" name="Amount" placeholder="Lodgement Amount" pattern="[0-9]+" title="Whole positive integer values " required>
	</fieldset>
	
	<input type="submit" id="Confirm" name="Confirm" value="Confirm" onclick="return makeLodgement(this)">
